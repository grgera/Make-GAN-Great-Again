...
class Attention(nn.Module):
    ...
    def __init__(self, attention_size):
        super().__init__()
        self.attention = self.new_parameter(attention_size * 2, 1)
    ...
    def forward(self, context, h):
        ...
        combined = torch.cat([context, h], dim=-1) 

        # Compute attention score
        attn_score = torch.matmul(combined, self.attention).squeeze(-1)
        attn_score = torch.softmax(attn_score, dim=1).unsqueeze(-1)   

        # Apply scores as weights & sum across timesteps
        scored_inp = context * attn_score                   
        feature = torch.sum(scored_inp, dim=1)           
...
class Generator(nn.Module):
    ...
    def sample(self, cond, gpu=False):
        ...
        return samples.view((num_samples, -1)), lens.view(-1) 
    ...
    def continue_sample_N(self, inp, n, h=None, context=None, gpu=False):
        ...
        samples[:, :sub_seq_len] = inp.unsqueeze(1).repeat(1, 1, n).view(n * batch_size, -1) 
        h = h.unsqueeze(1).repeat(1, 1, 1, n).view(1, -1, self.hidden_dim)                   
        out = inp[:, -1].unsqueeze(1).repeat(1, n).view(-1)                                  
        ...
        while not has_ended.all() and l < self.max_seq_len:             
            out, h = self.forward(out, h, context)
            out = self.sample_one(out)

            out[has_ended] = self.pad_token                             
            more_samples = torch.cat([more_samples, out.unsqueeze(0)])  
            lens[has_ended ^ 1] += 1                                   
        ...
        more_samples[has_ended ^ 1, -1] = self.end_token                
        more_seq_len = more_samples.shape[1]
        samples[:, sub_seq_len:sub_seq_len+more_seq_len] = more_samples 
